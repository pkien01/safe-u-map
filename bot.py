...
class DiscordBot(discord.Client):
    ...
    async def do_task(self):
        ...
        #print("Queue:",list(self.tasks_queue.queue))
        cnt = 0
        while not self.empty() and cnt < 5: 
            cnt += 1
            front, which_df = self.pop()
            notification = "%s\n%s\n\nExtracted locations:\n%s.\n\nIs this correct?" % (front['Date'].strftime("%Y-%m-%d %H:%M"), front['Content'], "\n".join(["\t\u2022 " + x[0] for x in front['Location']]))
            ...
            if msg.content == '!yes':
                ...
                for address in corrected_addresses:
                    location = self.geolocator.geocode(address.strip() + ", Minneapolis, MN",  timeout=10)
